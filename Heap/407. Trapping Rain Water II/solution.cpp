...
/*
    1. 先處理周圍的block, 因為周圍的block會直接流入大海，所以無法儲水

      [1], [4], [3], [1], [3], [2]
      [3],  2,   1,   3,   2,  [4]
      [2], [3], [3], [2], [3], [1]

    2. 因為minheap的關係，會從[1]開始往內部走，所以遇到了[3]，因為新的block[3]比[1]還高不可能儲水，水會往更低的地方流

      [1], [4], [3], [1], [3], [2]
      [3],  2,   1,  *3*,  2,  [4]
      [2], [3], [3], [2], [3], [1]

    3. 因為minheap的關係，接下來處理[2] 但是[2]周圍都處理過了，接下來處理[3]
    4. 因為minheap的關係，如果遇到比較低的block, 那目前的block就是四邊最低的block，所以可以儲的水就是cur - heightMap[y][x]
    ...
    5. 為什麼高度要取max?
       因為表示此路徑的最高block，下次遇到比較小的，它可以儲水的最高高度。
*/
